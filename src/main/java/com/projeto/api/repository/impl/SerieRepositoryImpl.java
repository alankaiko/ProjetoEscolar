...
import com.projeto.api.domain.Serie_;
...
public class SerieRepositoryImpl implements SerieRepositoryQuery{
	...
	public Page<Serie> Filtrando(SerieFilter filtro, Pageable pageable) {
		...
		TypedQuery<Serie> tiped = em.createQuery(query);
		AdicionarPaginacao(tiped, pageable);

		return new PageImpl<>(tiped.getResultList(), pageable, Total(filtro));
	}

	private Predicate[] AdicionarRestricoes(CriteriaBuilder builder, SerieFilter filtro, Root<Serie> root) {
		List<Predicate> lista = new ArrayList<Predicate>();

		if (!StringUtils.isEmpty(filtro.getSerie()))
			lista.add(builder.like(builder.lower(root.get(Serie_.serie)), "%" + filtro.getSerie().toLowerCase() + "%"));
